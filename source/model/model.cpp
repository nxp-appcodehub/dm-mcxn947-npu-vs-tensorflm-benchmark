...
static tflite::MicroOpResolver *s_op_resolver = nullptr;
...
status_t MODEL_Init(void)
{
    ...
    s_op_resolver = &micro_op_resolver;
...
status_t MODEL_Invoke(const uint8_t* model_data, uint32_t *during)
{
    ...
    tflite::MicroInterpreter interpreter(
        s_model, *s_op_resolver, s_tensorArena, kTensorArenaSize);
    ...
    size_t usedSize = interpreter.arena_used_bytes();
    PRINTF("\r\n%d/%d kB (%0.2f%%) tensor arena used\r\n", usedSize / 1024, kTensorArenaSize / 1024, 100.0*usedSize/kTensorArenaSize);
